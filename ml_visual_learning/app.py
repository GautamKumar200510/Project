...
def run_pipeline():
    try:
                    ...
                    if strategy == 'remove':
                        ...
                        if removed_count > 0:
                            for c in range(len(cols)):
                                cols[c] = [cols[c][i] for i in rows_to_keep]
        ...
        # Validate numeric features (Basic check to avoid 500 errors)
...
def export_dataset_part():
    try:
        ...
        # Suffix
        suffix = f"_{action_type}"
        if action_type == "range": suffix = "_part"
        ...
        with open(save_path, 'w', newline='') as f:
            writer = csv.writer(f)
            # User requirement: Do not include column names (header) in the export data
