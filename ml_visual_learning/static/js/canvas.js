document.addEventListener('DOMContentLoaded', () => {
    ...
    function createBlock(type, method, label, x, y, dataset_name = null, options = {}) {
        ...
        blockEl.innerHTML = `
            ${icon} <strong>${type !== 'dataset' ? label : label}</strong>
    ...
    function selectBlock(id) {
        ...
        if (blockObj) {
            ...
            if (type === 'dataset') {
                 ...
                 // For non-dataset blocks, hide the dataset info panel 
                 // unless we want to keep it persistent. Usually better to hide irrelevant info.
                 const datasetInfo = document.getElementById('dataset-info');
    ...
    function addDatasetToLibrary(filename, rows, cols) {
        ...
        div.innerHTML = `<i class="fas fa-table" style="margin-right:5px;"></i> ${filename} <div style="font-size:0.7em; margin-top:2px;">${rows}x${cols}</div>`;
...
function renderExcelTable(columns, rows) {
    ...
    columns.forEach((col, idx) => {
        const th = document.createElement('th');
        // Show Column Letter + Name
        th.innerText = col; 
        th.title = idxToCol(idx); // Tooltip shows Excel index
        trH.appendChild(th);
    });
...
function openCategoricalConfig(blockId) {
    ...
    .then(data => {
         ...
         populateCatColSelect(data.columns, block, datasetBlock.datasetFileName);
...
function openPreprocessingConfig(blockId) {
    ...
    const datasetBlock = findConnectedDataset(blockId);
    if (!datasetBlock) {
        alert("Please connect this block to a dataset first so we know the columns.");
        return;
    }
    ...
    .then(data => {
         if(data.error) { alert(data.error); return; }
         populatePreprocModal(data.columns, block);
...
function openStandardizerConfig(blockId) {
    ...
    .then(data => {
         if(data.error) { alert(data.error); return; }
         populateStdColSelect(data.columns, block, datasetBlock.datasetFileName);
